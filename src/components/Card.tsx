...
const Card: React.FC<CardProps> = (CardProps) => {
  return (
    <Link to={"/movie/" + CardProps.data.id} className="relative h-86 w-[230px] overflow-hidden rounded-lg">
      <img
      ...
      <div className="p-2 absolute text-white rounded-b-lg h-16 bottom-0 w-full max-w-[230px] bg-gradient-to-t from-gray-900 to-transparent">
        <h2 className="text-ellipsis line-clamp-1">{CardProps.data?.title || CardProps.data?.name}</h2>
        <div className="text-sm text-neutral-200 flex justify-between">
            <p>{CardProps.data.release_date}</p>
            <span className="bg-gray-900 rounded-full px-2 py-">Rating: {Number(CardProps.data.vote_average).toFixed(1)}</span>
        </div>
      </div>
    </Link>
  );
